    ...
    private:
        ...
        void DrawBatch(uint32_t bufferIndex, LineList::const_iterator& begin, LineList::const_iterator& end) {
            ...
            while (begin != end) {
                ...
                RE::NiCamera::WorldPtToScreenPt3((float(*)[4])g_worldToCamMatrix, *g_viewPort, a_start, start.x,
                                                 start.y, start.z, 1e-5f);
                start -= 0.5f;
                start *= 2.f;
                RE::NiPoint3 a_end{begin->end.pos.x, begin->end.pos.y, begin->end.pos.z};
                glm::vec3 _end{a_end.x, a_end.y, a_end.z};
                RE::NiCamera::WorldPtToScreenPt3((float(*)[4])g_worldToCamMatrix, *g_viewPort, a_end, _end.x, _end.y,
                                                 _end.z, 1e-5f);
                _end -= 0.5f;
                _end *= 2.f;
